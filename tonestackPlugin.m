classdef tonestackPlugin < audioPlugin
    properties
        Gain = 0.5;
        Out = 0.5;
    end
    ...
    methods
        function plugin = tonestackPlugin
            plugin.tonestack = tonestackModel(plugin.getSampleRate);
        end
        function Output = process(plugin, in)
            [sig, ~] = plugin.tonestack.process(in*plugin.Gain);
