...
function obj = pentode(name__, nodes__, value__)
obj.nodes = zeros(3,2); % some components can be three-ports like this
    obj.name = zeros(1,8);  % the name must have the same length 
    l = min(8,length(name__));
    obj.nodes(1:3,:) = nodes__; % set the nodes - three-port 
    obj.name(1:l) = name__(1:l); % set the name
    obj.value = value__; % set the value
    obj.type = 'ptd'; % 3char ID
    obj.model = '';
end
