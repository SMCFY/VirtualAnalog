...
void DiodeClipperAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    APDiode.initTree();
    APDiode.wdfTree::setSamplerate(sampleRate);
    APDiode.adaptTree();
}
...
void DiodeClipperAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    for (int i = 0; i < buffer.getNumSamples(); i++){
        ...
        APDiode.setInputValue(sample);
        APDiode.cycleWave();
        left[i] = APDiode.getOutputValue();
        right[i] = APDiode.getOutputValue();
    }
}
